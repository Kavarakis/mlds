...
class KernelTests(unittest.TestCase):
    ...
    def test_rbf(self):
        fitter = KernelizedRidgeRegression(kernel=RBF(sigma=0.5),
                                           lambda_=0.0001)
        m = fitter.fit(self.X, self.y)
        pred = m.predict(self.X)
        np.testing.assert_almost_equal(pred, self.y, decimal=3)
    ...
    def test_polynomial(self):
        fitter = KernelizedRidgeRegression(kernel=Polynomial(M=2),
                                           lambda_=0.0001)
        m = fitter.fit(self.X, self.y)
        pred = m.predict(self.X)
        np.testing.assert_almost_equal(pred, self.y, decimal=3)
...
class SVRTests(unittest.TestCase):
    ...
    def test_predictor_get_info(self):
        ...
        alpha = m.get_alpha()
        np.testing.assert_equal(alpha.shape,
                                (4, 2))  # two alpha for each sample
        # one value in row should be much bigger than the other (the other should be zero)
        np.testing.assert_allclose(alpha.sum(axis=1),
                                   alpha.max(axis=1),
                                   rtol=1e-5)
